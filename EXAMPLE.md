...
In this repository we provide two simple examples (example and example_grpc dirs)
...
1. mac address of default gateway: katran offloads all the routing to default gateway. It's doing so
...
for example: 
we have a server with 4 forwarding cpu 0,1,2,3. cpus 0 and 2 belongs to NUMA node 0
...
```
username@ubuntut2:~$ sudo tcpdump -ni enp0s8 proto 4 or host 10.200.200.1
tcpdump: verbose output suppressed, use -v or -vv for full protocol decode
